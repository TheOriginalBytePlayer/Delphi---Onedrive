...
begin
  ...
  try
    ...
    try
      ...
      begin
        FLog.Add('C�digo: ' + inttostr(e.ErrorCode) + #10#13 + 'Mensagem: ' + e.ErrorMessage);
...
function TDropBox.GetFoldersAsList(SortByName: boolean = false): TStringList;
...
function TDropBox.GetFilesAsList(SortByName: boolean = false): TStringList;
...
begin
  if FAcessCode = '' then
  begin
    raise Exception.Create('AcessCode n�o foi informado.');
  end;
end;
...
begin
  if FAcessToken = '' then
  begin
    raise Exception.Create('AcessToken n�o foi informado.');
  end;
end;
...
begin
  ...
  begin
    result := ('Token expirado ou inv�lido!');
  end;
  ...
  begin
    result := ('A aplica��o est� fazendo requisi��es demais e atingiu o limite of conex�es; Aguarde 10 segundos e tente novamente.: ' + e.Message);
  end;
...
procedure TDropBox.SetQtyParts(const Value: integer);
begin
  FQtyParts := Value;
end;
